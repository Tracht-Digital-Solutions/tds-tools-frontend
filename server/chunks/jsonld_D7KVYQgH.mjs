import { A as renderTemplate, At as number, D as renderSlot, Dt as _enum, F as createRenderInstruction, M as renderHead, Mt as string, N as addAttribute, Ot as array, P as defineScriptVars, T as Fragment$2, V as createAstro, j as maybeRenderHead, jt as object, kt as boolean, w as renderComponent, z as unescapeHTML } from "./sequence_BDmC7xT1.mjs";
import { t as createComponent } from "./compiler_Bp427A9v.mjs";
import { a as ogLocale, c as links, d as t, i as neutralPath, l as site, o as seoConfig, r as localizedPath, t as adsConfig } from "./catalog_BkMjpmD1.mjs";
...
var $$Footer = createComponent(async ($$result, $$props, $$slots) => {
	...
	})}</li>` : null}</ul></div>`)}</div><div class="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4 px-4 pt-4 pb-8 text-xs sm:px-6" style="border-top: 1px solid rgb(255 255 255 / 0.1); font-family: var(--font-mono); color: rgb(255 255 255 / 0.5);"><p>© ${year} Tracht Digital Solutions · ${s.footerTagline}</p>${adsEnabled && renderTemplate`<button type="button" id="tds-ad-consent-reset" class="btn btn-ghost" style="font-family: var(--font-mono); font-size: inherit;">${s.footerAdConsent}</button>`}</div>${adsEnabled && renderTemplate`<script>
      (function () {
