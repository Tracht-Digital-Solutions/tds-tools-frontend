import { A as renderTemplate, D as renderSlot, F as createRenderInstruction, M as renderHead, N as addAttribute, P as defineScriptVars, T as Fragment$2, V as createAstro, j as maybeRenderHead, w as renderComponent, z as unescapeHTML } from "./sequence_ByUFWzed.mjs";
import { t as createComponent } from "./compiler_052WwKvu.mjs";
import { a as ogLocale, c as links, d as t, i as neutralPath, l as site, o as seoConfig, r as localizedPath, t as adsConfig } from "./catalog_B2y3Hjla.mjs";
...
var $$Header = createComponent(($$result, $$props, $$slots) => {
	...
	})}<div class="tds-sitebar__wide"><a${addAttribute(contact, "href")} class="btn btn-primary no-underline">${s.cta}</a></div></div><div class="tds-sitebar__actions">${renderComponent($$result, "AccountMenu", AccountMenu, {
		"client:idle": true,
		"lang": lang,
		"loggedOut": "login",
		"client:component-hydration": "idle",
	...
	})}<button id="menu-toggle" type="button" class="btn btn-ghost tds-menu-toggle" aria-controls="mobile-menu" aria-expanded="false"${addAttribute(s.navMenu, "aria-label")}><span class="tds-menu-bar tds-menu-bar-top" aria-hidden="true"></span><span class="tds-menu-bar tds-menu-bar-mid" aria-hidden="true"></span><span class="tds-menu-bar tds-menu-bar-bot" aria-hidden="true"></span></button></div></div><div id="mobile-menu" class="tds-mobile-menu inset-x-0 top-[3.75rem]" style="--tds-mobile-menu-inset: 3.75rem" aria-hidden="true"${addAttribute(s.navMenu, "aria-label")}><nav aria-label="Navigation">${nav.map((item) => renderTemplate`<a${addAttribute(item.href, "href")} data-menu-link class="tds-mobile-menu__link"${addAttribute(item.current ? onCatalog ? "page" : "true" : void 0, "aria-current")}>${item.label}</a>`)}</nav><div class="mt-3 flex flex-wrap items-center justify-between gap-2 border-t border-[color:var(--color-line)] pt-3"><div class="flex items-center gap-2">${renderComponent($$result, "ThemeToggle", ThemeToggle, {
		"client:idle": true,
		"client:component-hydration": "idle",
		"client:component-path": "@tracht-digital-solutions/tds-shared/components",
		"client:component-export": "ThemeToggle"
	})}${renderTemplate`<div class="tds-lang-toggle" role="group" aria-label="Sprache / Language">${langHrefs.map((l) => renderTemplate`<a${addAttribute(l.href, "href")}${addAttribute(l.code, "hreflang")}${addAttribute(l.code, "lang")} data-menu-link${addAttribute(l.code === lang ? "on" : "", "class")}${addAttribute(l.code === lang ? "true" : void 0, "aria-current")}>${l.label}</a>`)}</div>`}</div><a${addAttribute(contact, "href")} data-menu-link class="btn btn-primary no-underline">${s.cta}</a></div></div></header><span class="sr-only" data-site-tagline>${s.tagline}</span>${renderScript($$result, "/home/runner/work/tds-tools-frontend/tds-tools-frontend/src/components/Header.astro?astro&type=script&index=0&lang.ts")}`;
...
//#endregion
export { itemListSchema as a, softwareApplicationSchema as c, $$AdSlot as d, $$Icon as f, renderScript as h, howToSchema as i, websiteSchema as l, runtimeSetting as m, breadcrumbSchema as n, organizationSchema as o, $$Layout as p, faqPageSchema as r, personSchema as s, asGraph as t, $$ServiceNote as u };
